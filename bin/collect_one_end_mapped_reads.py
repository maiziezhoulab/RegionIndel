...
def search_one_chr(mate_ref,qnames,chr_name):
	dc_mate_one_chr = {}
	logger.info("search "+ mate_ref)
	qnames = set(qnames)
	pickle_path = feedback_dir + "/feedback_from_%s_to_%s.p"%(mate_ref, chr_name)
...
sequences = Parallel(n_jobs=n_thread)(delayed(search_one_chr )(mate_ref, dc_by_chr[mate_ref],chr_name) for mate_ref in tqdm(dc_by_chr))
