...
import time
from Concatenate_contigs_all_extend_for_HCbk import *
...
def use_spades(one_file_fastq,spades_path,out_dir,xin):
   # try:
    ...
    use_cmd = "python2 "+spades_path+"/bin/spades.py -t " + str(num_threads_spades) + " --only-assembler --12 " + one_file_fastq + " -o " + out_dir 
...
def run_spades_all(chr_start,chr_end,output_dir,num_of_threads,minicontig_dir):
    ...
    for chr_num in range(chr_start, chr_end+1):
        ...
        count = 1
        fastq_files_all = sorted(glob.glob(in_dir +  "fastq_by_*_add_OER.fa"),key=os.path.getsize,reverse=True)
        ...
        for one_file_fastq in fastq_files_all:
            ...
            if os.path.exists(spades_contig_file):
                ...
                count += 1
                pool.apply_async(use_spades,(one_file_fastq,spades_path,out_dir,"xin"))
                out_dir_list.append(out_dir)
