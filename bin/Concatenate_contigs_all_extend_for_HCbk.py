...
def concatenate_contigs(contig_file,fw,contig_num,contigs_all,hp_flag,PS_flag):
    ...
    for line in f:
        ...
        if data[0][0] == ">":
            if flag == 1:
                fw.writelines(">" + str(contig_num) + "_PS" +  str(PS_flag)  + "_hp" + str(hp_flag) + "\n")
...
def concatenate_all(input_dir,output_filename):
    ...
    for one_file in glob.glob(input_dir + "fastq_by_*_spades_assembly"):
        ...
        if os.path.exists(spades_contig_file):
            ...
            fw_contigs_all, contig_num,contigs_all = concatenate_contigs(spades_contig_file,fw_contigs_all,contig_num,contigs_all,hp_flag,PS_flag)
        else:
            count_3 += 1
